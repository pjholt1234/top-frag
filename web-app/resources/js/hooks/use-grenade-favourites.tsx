...
export const useGrenadeFavourites = () => {
  const [favouritedGrenades, setFavouritedGrenades] = useState<Set<string>>(
    new Set()
  );
  const [favouriteIds, setFavouriteIds] = useState<Map<string, number>>(
    new Map()
  );
  ...
  const checkFavouriteStatus = useCallback(
    async (grenade: GrenadeData): Promise<FavouriteStatus> => {
      try {
        const response = await api.get<FavouriteStatus>(
          '/grenade-favourites/check',
          {
            params: {
