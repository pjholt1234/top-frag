...
const DEFAULT_CONTEXT_VALUE: UseGrenadeLibraryReturn = {
  // Data
  grenades: [],
  ...
  currentMap: 'de_ancient',
};

// Context
const GrenadeLibraryContext = createContext<UseGrenadeLibraryReturn>(
...
export const GrenadeLibraryProvider: React.FC<GrenadeLibraryProviderProps> = ({
  children,
  initialFilters = {},
}) => {
  ...
  const loadFilterOptions = useCallback(async () => {
    try {
      ...
      const response = await get<FilterOptions>(
        '/grenade-favourites/filter-options'
      );
  ...
  const loadGrenades = useCallback(async () => {
    ...
    try {
      ...
      const response = await get<{ grenades: FavouritedGrenadeData[] }>(
        '/grenade-favourites',
        { params }
      );
      ...
      toast.promise(
        (async () => {
          try {
            ...
            setGrenades(prev =>
              prev.filter(grenade => grenade.id !== favouriteId)
            );
