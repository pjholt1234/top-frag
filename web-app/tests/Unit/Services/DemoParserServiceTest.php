...
class DemoParserServiceTest extends TestCase
{
    ...
    public function test_it_handles_match_time_extraction_failure_gracefully(): void
    {
        ...
        $logChannel->shouldReceive('warning')
            ->once()
            ->with('Failed to extract match start time', \Mockery::type('array'));

        // Also mock the ProcessClanMatch dispatch warning if it fails
        \Illuminate\Support\Facades\Log::shouldReceive('warning')
            ->zeroOrMoreTimes()
            ->with('Failed to dispatch ProcessClanMatch job', \Mockery::type('array'));
